...
void getSum(const int& numPoints, LL& sumX, LL& sumY, LL& sumSquare)
{ 
    sumX = 0; sumY = 0; sumSquare = 0;          // Reset values
    for (LL i = 0, x, y; i < numPoints; i++) {
        cin >> x >> y;          // Coordinates: 0 ≤ x, y ≤ 1e9
        sumX = (sumX + x) % MOD;
        sumY = (sumY + y) % MOD;
...
LL solve()
{
    ...
    getSum(N, sumA, sumB, sumSquare1);
    int Q; cin >> Q;                // No. wells K: 0 ≤ K ≤ 500,000
    getSum(Q, sumX, sumY, sumSquare2);
